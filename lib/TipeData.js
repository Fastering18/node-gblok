...
class Objek extends Isi {
  ...
  inspek(obj) {
    return `{${Object.keys(obj).map((v) => `${v}: ${obj[v] instanceof Objek ? this.inspek(obj[v].nilai) : obj[v].publik2 || obj[v].publik || obj[v].nilai}`).join(", ")}}`
  }

  get publik() {
    return this.inspek(this.nilai)
  }
}
...
class BaseFungsi extends Isi {
  ...
  buat_konteks_baru() {
    ...
    konteks_baru.lokasi = this.kontes ? this.konteks.lokasi : ""
...
class BuiltInFungsi extends BaseFungsi {
  ...
  esekusi_impor(konteks_esekusi, { Lexer, Parser, Interpreter, konteks }) {
    ...
    if (!(nama_module instanceof Str)) return res.gagal(new RTError(
      this.posisi_awal, lain.posisi_akhir, "Lokasi file harus berupa string", this.konteks
    ))

    var abs_path_modul = path.join(process.cwd(), konteks.lokasi, "..", nama_module.nilai)
    //console.log(abs_path_modul, "\n", process.cwd(), konteks.lokasi, nama_module.nilai)
    if (!fs.existsSync(abs_path_modul)) return res.gagal(new RTError(
    ...
    const lex = new Lexer(path.basename(abs_path_modul), isi_script.toString())
    var { hasil, err } = lex.buatToken()
