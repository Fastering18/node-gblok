...
const { tunggu, sleep, deepEqual } = require("./utility")
const { getPackageFromDirectory } = require("../cmd/projek")
var readlineSync = require('readline-sync');
const fs = require("fs")
const path = require("path")
//const r = require("../src/interpreter")
  ...
  switch (typeof data) {
    ...
    case "object":
      if (Array.isArray(data)) { 
        hasil = new Daftar(data.map(convert_tipe));
      } else {
        //let gblkdata = {}
        //for (var k in data) gblkdata[k] = convert_tipe(data[k]);
        hasil = new Objek(data);
      }
      break
    default:
      hasil = new Nil()
...
function convert_tofunction_nodejs(fn) {
  ...
  var hasil = (...params) => {
    let gblkparams = params.map(nativeBindingVariable)
    let fnresult = fn.esekusi(gblkparams, {Interpreter})
...
class Daftar extends Isi {
  ...
  perbandingan_persamaan(lain) {
    var hasil = new BooLean((lain instanceof Daftar || lain instanceof Objek)? deepEqual(this, lain) : false).atur_konteks(this.konteks)
    return { hasil: hasil, err: null }
  }
  ...
  perbandingan_kurang_dari(lain) {
    if (!(lain instanceof Daftar)) return { hasil: null, err: this.operasi_illegal(lain) };

    var hasil = new BooLean(this.nilai.length < lain.nilai.length).atur_konteks(this.konteks)
    return { hasil: hasil, err: null }
  }
  ...
  bukan() {
    return { hasil: new BooLean(0).atur_konteks(this.konteks), err: null }
  }
